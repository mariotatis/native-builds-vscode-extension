...
export interface ProductInfo {
  ...
  appPath: string;
  /** Bundle identifier (needed to launch on simulators / devices). */
  bundleId?: string;
}

/**
 * Read build settings for the chosen scheme + destination and locate the
 * product .app plus its bundle id. Uses the JSON form for reliable parsing.
 */
...
/** Install (if needed) and launch the product on the selected destination. */
export async function launch(
  ...
  if (dest.group === "mac") {
    // Force a clean relaunch: terminate any running instance, then open.
    const killed = await run(
      ...
      .then(() => true)
      .catch(() => false); // exit 1 = nothing matched, that's fine
    if (killed) {
      await sleep(400); // let the old instance finish quitting
    }
  ...
  // Physical device — requires Xcode 15+ (devicectl).
  ...
  // --terminate-existing relaunches even when the app is already running.
  await run(
    "xcrun",
    [
      "devicectl",
...
async function launchOnSimulator(
  ...
  requireBundleId(product);
  // --terminate-running-process relaunches even when the app is already running.
  await run(
    "xcrun",
